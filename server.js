...
app.get('/checkAlert', function(req, res) {
  ...
  connection.query("SELECT userinfotable.first_name, userinfotable.last_name, userinfotable.device_address, userinfotable.team, useralerttable.time_of_request, CONVERT_TZ(useralerttable.time_of_request,'GMT','-6:00') AS time_of_request FROM userinfotable, useralerttable WHERE useralerttable.device_address_sender = userinfotable.device_address AND useralerttable.device_address_receiver = '" + req.headers.deviceaddress + "';", function (error, results, fields) {
    if(error) {
      res.send({
